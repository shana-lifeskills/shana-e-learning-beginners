...
import { HygieneFillBlankSentence, HygieneFillBlankStep } from '../../core/models/module.model';

/**
 * "Warm-Up – Fill in the Blank" game (Hygiene Module, Week 4). Tap a word from
 * the bank then the blank it belongs in: the right word plants itself and its
 * tile leaves the bank, a wrong word shakes the sentence and stays available.
 * Continue unlocks once every blank is filled. Its own visual design — see
 * `HygieneFillBlankStep` in `module.model.ts`.
 */
@Component({
  selector: 'app-hygiene-fill-blank-step-view',
  standalone: true,
  templateUrl: './hygiene-fill-blank-step-view.html',
  styleUrl: './hygiene-fill-blank-step-view.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class HygieneFillBlankStepView implements OnInit {
  readonly step = input.required<HygieneFillBlankStep>();
  ...
  isFilled(sentence: HygieneFillBlankSentence): boolean {
  ...
  placeInBlank(sentence: HygieneFillBlankSentence): void {
